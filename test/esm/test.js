...
test('DrissionPage Detector', async () => {
  ...
  const result = await page.evaluate(() => { return !!document.querySelector('#isBot span').textContent.includes('not') })
  await browser.close()
...
test('Brotector, a webdriver detector', async () => {
  ...
  const result = await page.evaluate(() => { return document.querySelector('#table-keys').getAttribute('bgcolor') })
  await browser.close()
